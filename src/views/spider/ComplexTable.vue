      ...
      >
        ...
        <el-table-column label="任务操作" align="center" width="280" class-name="small-padding fixed-width">
          <template slot-scope="{row,$index}">
            ...
            <el-button v-if="row.status!='published' && row.status!='doing'&&row.status!='done'" 
                size="mini" type="success" @click="handleModifyStatus(row,'doing')">
              开始
            </el-button>
            <el-button v-if="row.status!='draft'&&row.status!='done'" size="mini" @click="handleModifyStatus(row,'draft')">
              暂停
            </el-button>
  ...
  export default {
    ...
    data() {
      return {
        ...
        statusOptions: ['published', 'draft', 'deleted','doing','done'],
        statusName:{
            "published":"开始",
            "draft":"暂停",
            "deleted":"删除",
        ...
        statusType:[
            ...
            { key: 'draft', display_name: '暂停' },
            { key: 'done', display_name: '已完成' },
        ],
        showReviewer: false,
        temp: {
          id: undefined,
